...
class FireProblem(object):
    ...
    ----------
    T_1 : scalar(int), optional(default=15)
    ...
    ----------
    xi : np.ndarray
        Match quality signals, ndim = n x T_max
    c : np.ndarray
    ...
    def __init__(self, w=40, beta=0.95, y_0=-1, sig2_0=0.5, 
                 sig2_star=2, T_max=360, T_1=15, T_2=30,
                 b=0.358, f=1/300, sp=2, st=0.1):
        ...
        #Grid setup
        self.n = 1
        self.y = np.arange((y_0+w/10)-(np.ceil(100*(max(sig2_0,sig2_star))**(0.5))/100)*sp, 
             (y_0+w/10)+(np.ceil(100*(max(sig2_0,sig2_star))**(0.5))/100)*sp, st) # Belief grid
    ...
    def bellman_operator(self, v):
        ...
        v_cols = np.empty(self.n*self.y.size*self.t.size, dtype=object)
        it = 0
        ...
        E0 = np.array([integrate.fixed_quad(lambda x: v_cols[l][0](x)*self.h_grid[l](x), i-3*j**0.5, i+3*j**0.5)[0] 
             for l, (i, j) in enumerate(zip(self.mu_grid.flatten(),self.sigma_grid.flatten()))]).reshape(self.n,self.y.size,self.t.size)
        v0 = self.beta*E0

        #fire worker
        E1 = np.array([integrate.fixed_quad(lambda x: v_cols[l][0](x)*self.h_0(x), 
